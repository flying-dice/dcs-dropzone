...
describe("inferReleaseStatusFromAssets", () => {
	...
	it("returns DISABLED when all assets are COMPLETED and enabled is false (empty assets)", () => {
		const assetStatus: AssetStatus[] = [];

		const result = inferReleaseStatusFromAssets(assetStatus, false);
		// Empty assets means .every() returns true (vacuously true for PENDING check),
		// so status is PENDING; but this edge case is unlikely in practice
		expect(result).toBe(DownloadedReleaseStatus.PENDING);
	});
