...
export type ExtractQueueOrchestratorConfig = {
	...
	downloadQueue: DrizzleWgetDownloadQueue;
};

export class Drizzle7zExtractQueue implements ExtractQueue {
	...
	getJobsForReleaseId(releaseId: string): {
		...
		return this.db.select().from(T_EXTRACT_QUEUE).where(eq(T_EXTRACT_QUEUE.releaseId, releaseId)).all();
	}
	...
	getJobsForReleaseAssetId(releaseAssetId: string): {
		...
		return this.db.select().from(T_EXTRACT_QUEUE).where(eq(T_EXTRACT_QUEUE.releaseAssetId, releaseAssetId)).all();
	}
}
